...
export async function analyzeWithOpenAI(flights, travelClass) {
  // Enrich each flight with estimated Amex miles options
  const enrichedFlights = flights.map(flight => ({
    ...flight,
    amex_programs: estimateMilesOptions(flight),
  }));

  // Main prompt for GPT-5
  const prompt = `
...
`;
  ...
  const completion = await openai.chat.completions.create({
    model: "gpt-5-turbo",
    messages: [
      { role: "system", content: "You are a precise financial travel assistant." },
  ...
  try {
    ...
    console.error("⚠️ GPT parsing error:", err);
    return [
      { error: "Failed to parse AI output", raw: completion.choices[0].message.content },
    ];
  }
}
